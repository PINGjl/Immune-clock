...
valid.rows_all <- sampled_prop$donor
train.rows_all <- setdiff(inf$donor, sampled_prop$donor)
...
cellprop <- dcast(cellprop, donor ~ subtype, value.var = 'prop')
rownames(cellprop) <- cellprop$donor
cellprop <- cellprop[,-1]
cellprop <- (cellprop - min(cellprop, na.rm=T)) / (max(cellprop, na.rm=T) - min(cellprop, na.rm=T))
cellprop$donor <- rownames(cellprop)
df <- cbind(df,cellprop)
df <- df[,-1]
...
for (i in celltypes){
  meta <- read.csv(paste0("E:/01_progrom/06_qz_pbmc_new/05_gene/expression_donor/cor_age_gender_sample_expression/",i,"_cor_gene_meta.csv"),row.names = 1,check.names = FALSE)
  meta <- meta[,c(-1,-2,-3)]
  aa <- grep("^HIST", setdiff(colnames(meta), annogene$gene_name), value = TRUE, invert = TRUE)
  meta <- meta[, -which(colnames(meta) %in% aa)]
  
  meta <- (meta - min(meta, na.rm=T)) / (max(meta, na.rm=T) - min(meta, na.rm=T))
  colnames(meta) <- paste0(i,'_',colnames(meta))
  meta$donor <- rownames(meta)
  df <- merge(df,meta, by = 'donor', all.x = T)
} 

meta <- read.csv("E:/01_progrom/06_qz_pbmc_new/04_clock_gender/03_tcr/TCR_data_scale_sign.csv")
df <- merge(df,meta, by = 'donor')#, all.x = T) #
df[is.na(df)] <- 0

inf <- read.csv("E:/01_progrom/06_qz_pbmc_new/00_inf/inf_merge_final_me.csv")
inf <- inf[,c('donor','group','Age', 'gender')]
df <- merge(inf, df, by= 'donor')
df$gender <- as.numeric(as.factor(df$gender)) -1

wtmeta <- subset(df, group %in% "control")

valid.rows <- intersect(valid.rows_all,wtmeta$donor)
train.rows <- intersect(train.rows_all,wtmeta$donor)

mae.df=c()
result <- 'E:/01_progrom/06_qz_pbmc_new/04_clock_gender/04_combind/all/'
...
######bingxing#####
tc <- c("Tn_CD4", "Tcm_CD4","Th1_CD4","Th2_CD4","Th17_CD4","Treg","Tex_CD4",
        "Tn_CD8",'Tcm_CD8',"Tem","TCR","CTL","Tex_CD8",'proT')
nk <- c('NK1','NK2')
bc <- c('Bn','Bm','Pla')
mc <- c('cMC','nMC')
dc <- c('cDC','pDC')
anno <- c('tc','nk','bc','mc')#,'dc')
#int.list <- list(get(anno[1]),get(anno[2]),get(anno[3]),get(anno[4]),get(anno[5]))

registerDoParallel(cores = detectCores() - 1)
packs <- c("Hmisc",'ggplot2','glmnet','dplyr','ggthemes','limma','cowplot','reshape2')

foreach(cell = anno, .packages = packs, .export = c('tc','nk','bc','mc')) %dopar% {
  df = as.data.frame(matrix())
  cellprop <- read.csv("E:/01_progrom/06_qz_pbmc_new/03_celltype/final_celltype_prop.csv",row.names = 1)
  sign <- read.csv("E:/01_progrom/06_qz_pbmc_new/03_celltype/final_celltype_pcor.csv")
  sign <- subset(sign, p.value < 0.05)
  cellprop <- subset(cellprop, subtype %in% sign$subtype)
  cellprop <- subset(cellprop, subtype %in% get(cell))
  cellprop <- subset(cellprop, prop > 0)
  cellprop <- dcast(cellprop, donor ~ subtype, value.var = 'prop')
  rownames(cellprop) <- cellprop$donor
  
  cellprop <- cellprop[,-1, drop = FALSE]
  cellprop <- (cellprop - min(cellprop, na.rm=T)) / (max(cellprop, na.rm=T) - min(cellprop, na.rm=T))
  cellprop$donor <- rownames(cellprop)
  df <- cbind(df,cellprop)
  df <- df[,-1]
  ...
  mae.df=c()
  dir.create(paste0('E:/01_progrom/06_qz_pbmc_new/04_clock_gender/04_combind/',cell,'/'))
